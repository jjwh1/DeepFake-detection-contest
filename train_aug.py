...
# ===================== 추가 =====================
# AmpSwap / AmpMix augmentation
import torch.fft as fft

def ampswap_batch(x, r=0.05):
    B, C, H, W = x.shape
    ...
    mag[:, :, cy-rh:cy+rh, cx-rw:cx+rw] = mag_swap[:, :, cy-rh:cy+rh, cx-rw:cx+rw]

    F_new = mag * torch.exp(1j * phase)
    out = fft.ifft2(fft.ifftshift(F_new, dim=(-2, -1)), dim=(-2, -1)).real
    return out


def ampmix_batch(x, eta=0.3):
    B, C, H, W = x.shape
    F = fft.fftshift(fft.fft2(x, dim=(-2, -1)), dim=(-2, -1))
    mag = F.abs()
    phase = torch.angle(F)
    ...
    lam = torch.rand(B, 1, 1, 1, device=x.device) * eta
    mag_new = (1 - lam) * mag + lam * mag_mix

    F_new = mag_new * torch.exp(1j * phase)
    out = fft.ifft2(fft.ifftshift(F_new, dim=(-2, -1)), dim=(-2, -1)).real
    return out
# ===================== 추가 끝 =====================
    ...
    for step, (imgs, targets) in enumerate(tqdm(loader, desc=f"Train [{epoch+1}]")):
        ...
        if torch.rand(1).item() < 0.85:
            ...
            if B >= 2:
                ...
                # swap or mix 랜덤 선택
                if torch.rand(1).item() < 0.85:
                    pair_imgs = ampswap_batch(pair_imgs, r=0.08)
                else:
                    pair_imgs = ampmix_batch(pair_imgs, eta=0.4)

                imgs = torch.cat([pair_imgs, imgs[B:]], dim=0)
        # ===================== 추가 끝 =====================
...
def main():
    ...
    parser.add_argument("--outdir",    type=str, default="/content/drive/MyDrive/DeepFake_detection/result/1_aug")
